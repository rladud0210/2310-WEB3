...
prevButton.addEventListener('click', ()=>{
    ...
    if(index === 한번에보는슬라이드 - 1) {
        ...
        // 예약 실행 시켜주는 함수
        // setTimeout(()=>{
        // 예약 실행할 부분. 작성된 코드가 정해진 시간 뒤에 실행된다
...
// 인덱스에 따라 슬라이드가 움직이게 만들고 true, false를 전달받아 애니메이션 실행 여부를 결정할 수 있다.
// 이걸 실행시키지 않으면 인덱스만 변함
function 위치적용(애니메이션여부) {
   ...
   // 사용자 기준 슬라이드 인덱스
    ...
    // 초는 문자열을 직접 넣는게 아니라, 상수[애니메이션] 를 활용해 설정해봅시다.

    // contentsWrapper.style.transition = `1s`

    if (애니메이션여부) {
        contentsWrapper.style.transition = `${애니메이션속도}ms` 
    } else { contentsWrapper.style.transition = `0ms`}
}
